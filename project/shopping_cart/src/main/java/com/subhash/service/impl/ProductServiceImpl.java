...
public class ProductServiceImpl implements ProductService {
	...
	public List<ProductBean> getAllProductsByType(String type) {
		...
		try {
			ps = con.prepareStatement("SELECT * FROM `shopping_cart`.product where lower(ptype) like ?;");
			ps.setString(1, "%" + type + "%");
			rs = ps.executeQuery();
	...
	public List<ProductBean> searchAllProducts(String search) {
		...
		try {
			ps = con.prepareStatement(
					"SELECT * FROM `shopping_cart`.product where lower(ptype) like ? or lower(pname) like ? or lower(pinfo) like ?");
			search = "%" + search + "%";
